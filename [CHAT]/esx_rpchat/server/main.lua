...
function getIdentity(source)
  ...
  AddEventHandler('chatMessage', function(source, name, message)
	  if string.sub(message, 1, string.len("/")) ~= "/" then
		...
		TriggerClientEvent("sendProximityMessage", -1, source, name, msg)
      end
      CancelEvent()
  end)
...
AddEventHandler('3dme:shareDisplayMe', function(text)
	...
	msg = Emojit(text)
	TriggerEvent("esx_discord_bot:log", 'ME', '[' .. source .. ' | ' .. xPlayer.identifier .. ' | ' .. GetPlayerName(source) .. '] /me ' .. msg)
end)
...
AddEventHandler('3dme:shareDisplayDo', function(text)
	...
	msg = Emojit(text)
	TriggerEvent("esx_discord_bot:log", 'DO', '[' .. source .. ' | ' .. xPlayer.identifier .. ' | ' .. GetPlayerName(source) .. '] /do ' .. msg)
end)
    ...
    if pID ~= nil then
        if GetPlayerPing(pID) == 0 then
            ...
            if args[2] ~= nil then
                ...
                local msg = string.gsub(text, args[1] .. ' ', "")
                TriggerClientEvent("sendMessageToPlayer", -1, source, name, msg, source)
                TriggerClientEvent("sendMessageToPlayer", -1, pID, name, msg, source)
            else
                TriggerClientEvent("pNotify:SendNotification", source, {
                    text = "Podej treść którą chcesz wysłać do danej osoby!",
                    type = "success",
                    queue = "lmao",
                    timeout = 2000,
                    layout = "centerLeft"
        ...
        TriggerClientEvent("pNotify:SendNotification", source, {
            text = "Podaj ID osoby do której chcesz napisać!",
            type = "success",
            queue = "lmao",
            timeout = 2000,
            layout = "centerLeft"
...
TriggerEvent('es:addCommand', 'twt', function(source, args, user)
	...
	if args[1] ~= nil then
		...
		msg = Emojit(table.concat(args, " "))
		local mahajs1 = xPlayer.getMoney()
		if mahajs1 > 10 then
			TriggerClientEvent("pokaTwT", -1, source, name.firstname .. " ".. name.lastname, msg)
...
TriggerEvent('es:addCommand', 'lspd', function(source, args, user)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'police' then
			...
			local mahajs1 = xPlayer.getMoney()
			if mahajs1 > 200 then
				TriggerClientEvent("pokaLSPD", -1, -1, name.firstname .. " ".. name.lastname, msg)
...
TriggerEvent('es:addCommand', 'ogloszenie', function(source, args, user)
	if args[1] ~= nil then
		local user = getIdentity(source)
		if user.group ~= 'user' then
			local playerName = GetPlayerName(source)
			msg = Emojit(table.concat(args, " "))
			TriggerClientEvent("pokaOGL", -1, -1, playerName, msg)
		end
	end
end)
...
TriggerEvent('es:addCommand', 'dw', function(source, args, user)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	if (xPlayer.job.name == "police" or xPlayer.job.name == "offpolice" or xPlayer.job.name == "ambulance" or xPlayer.job.name == "offambulance") then
		TriggerClientEvent("pNotify:SendNotification", source, {
		...
		TriggerEvent("es:getPlayers", function(pl)
			...
			if (xPlayer.job.name == "police" or xPlayer.job.name == "offpolice" or xPlayer.job.name == "ambulance" or xPlayer.job.name == "offambulance") then
				...
				local mahajs1 = xPlayer.getMoney()
				if mahajs1 > 200 then
					TriggerClientEvent("pokaDw", -1, -1, msg)
					xPlayer.removeMoney(200)
					TriggerClientEvent('esx:showNotification', source, '~y~Pobrano opłatę~r~ 200$')
				else
					TriggerClientEvent('esx:showNotification', source, '~r~Nie masz pieniędzy!')
...
end, {help = 'Wyslij wiadomosc na darkweb. [DW]'})
